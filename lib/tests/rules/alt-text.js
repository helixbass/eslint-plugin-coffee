...
(function() {
  /* eslint-env jest */
  var RuleTester, altValueError, areaError, array, inputImageError, missingPropError, objectError, parserOptionsMapper, path, preferAltError, rule, ruleTester;
  ...
  missingPropError = function(type) {
    return {
  ...
  altValueError = function(type) {
    return {
      message: `Invalid alt value for ${type}. Use alt="" for presentational images.`,
      type: 'JSXOpeningElement'
    };
  };
  ...
  objectError = 'Embedded <object> elements must have alternative text by providing inner text, aria-label or aria-labelledby props.';

  areaError = 'Each area of an image map must have a text alternative through the `alt`, `aria-label`, or `aria-labelledby` prop.';

  inputImageError = '<input> elements with type="image" must have a text alternative through the `alt`, `aria-label`, or `aria-labelledby` prop.';
