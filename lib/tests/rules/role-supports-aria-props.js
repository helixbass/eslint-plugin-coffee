...
(function() {
  /* eslint-env jest */
  var RuleTester, aria, createTests, errorMessage, generateErrorMessage, invalidTests, nonAbstractRoles, parserOptionsMapper, path, roles, rule, ruleTester, validTests;
  ...
  generateErrorMessage = function(attr, role, tag, isImplicit) {
    if (isImplicit) {
  ...
  errorMessage = function(attr, role, tag, isImplicit) {
    return {
  ...
  nonAbstractRoles = [...roles.keys()].filter(function(role) {
    return roles.get(role).abstract === false;
  });

  createTests = function(rolesNames) {
    return rolesNames.reduce(function(tests, role) {
      var allTests, invalidPropsForRole, normalRole, propKeyValues, validPropsForRole;
      ({
        props: propKeyValues
      } = roles.get(role));
      validPropsForRole = Object.keys(propKeyValues);
      invalidPropsForRole = [...aria.keys()].map(function(attribute) {
        return attribute.toLowerCase();
      }).filter(function(attribute) {
        return validPropsForRole.indexOf(attribute) === -1;
      });
      normalRole = role.toLowerCase();
      allTests = [];
      allTests[0] = tests[0].concat(validPropsForRole.map(function(prop) {
        return {
          code: `<div role=\"${normalRole}\" ${prop.toLowerCase()} />`
        };
      }));
      allTests[1] = tests[1].concat(invalidPropsForRole.map(function(prop) {
        return {
          code: `<div role=\"${normalRole}\" ${prop.toLowerCase()} />`,
