...
(function() {
  ...
  CodePathState = class CodePathState {
     ...
     */
    popChoiceContext() {
      var context, forkContext, headSegments, parentContext, prevForkContext;
    ...
    makeLogicalRight() {
       ...
       */
      var context, forkContext, prevForkContext;
      context = this.choiceContext;
      forkContext = this.forkContext;
      if (context.processed) {
        /* kind === "||" */
        prevForkContext = context.kind === '&&' ? context.trueForkContext : context.falseForkContext;
        forkContext.replaceHead(prevForkContext.makeNext(0, -1));
    ...
    makeIfConsequent() {
      ...
      return forkContext.replaceHead(context.trueForkContext.makeNext(0, -1));
    }
    ...
    makeIfAlternate() {
      ...
      return forkContext.replaceHead(context.falseForkContext.makeNext(0, -1));
    }
    ...
    popSwitchContext() {
      ...
      forkContext = this.forkContext;
      brokenForkContext = this.popBreakContext().brokenForkContext;
    ...
    makeContinue(label) {
      ...
      if (context) {
        if (context.continueDestSegments) {
          ...
          if ((ref = context.type) === 'ForInStatement' || ref === 'ForOfStatement') {
            context.brokenForkContext.add(forkContext.head);
          }
        } else {
          context.continueForkContext.add(forkContext.head);
