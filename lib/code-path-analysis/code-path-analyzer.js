...
(function() {
  ...
  forwardCurrentToHead = function(analyzer, node) {
    var codePath, currentSegment, currentSegments, end, headSegment, headSegments, i, j, k, ref, ref1, results, state;
    ({codePath} = analyzer);
...
// Fires entering events.
    ...
    for (i = k = 0, ref1 = end; (0 <= ref1 ? k < ref1 : k > ref1); i = 0 <= ref1 ? ++k : --k) {
      ...
      if (currentSegment !== headSegment && headSegment) {
        ...
        if (headSegment.reachable) {
          results.push(analyzer.emitter.emit('onCodePathSegmentStart', headSegment, node));
        } else {
          results.push(void 0);
        }
      } else {
  ...
  preprocess = function(analyzer, node) {
    var codePath, parent, state;
    ({codePath} = analyzer);
    state = CodePath.getState(codePath);
    ({parent} = node);
    switch (parent.type) {
